...
function determinantForArray() {
	...
	var minuspos = str.indexOf("-");
		...
		var afterSlashFraction = str.substr(str.indexOf("/")+1);
		end = "{";
		var Fraction = Number(afterSlashFraction);
		var abc = math.fraction(Fraction);
		abc = math.fraction({
			n: abc.n,
			d: abc.d
		});
		...
		if(abc.d ===1){
			...
			var frac = "\\frac {" + start + "}";
			var fracEnd = "{" + abc.n + "}";
			answerString += "(-"+ frac + fracEnd + ")";
		}else{
		var numerator = "\\frac {" + abc.n + "}";
		var denominator = "{" + abc.d + "}";
		var FractionAnswer = numerator + denominator;
		var finalFractionAnswer = "(-" + start + end + numerator + denominator + "}" + ")";
		answerString = finalFractionAnswer;
		}
